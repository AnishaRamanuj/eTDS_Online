<%@ WebHandler Language="C#" Class="Xl27Q" %>
...
public class Xl27Q  : DataLayer.DALCommon, IHttpHandler, IRequiresSessionState
{
        ...
        try
        {
            ...
            if (context.Request.Files.Count > 0)
            {
                ...
                using (OleDbConnection excel_con = new OleDbConnection(conString))
                {
                    ...
                    //string Sheet1 = dtSchema.Rows[2].Field<string>("TABLE_NAME");
                    string Sheet1 = "";
                    foreach (DataRow dr in dtSchema.Rows)
                    {
                        Sheet1 = dr["TABLE_NAME"].ToString();

                        if (Sheet1 == "'Deductee Details$'")
                        {         
                            df = "Deductee Details$";
                            // Sheet1 = dtSchema.Rows[0].Field<string>("TABLE_NAME"); //"DeducteeDetails$";
                            using (OleDbDataAdapter oda = new OleDbDataAdapter(String.Format("SELECT * FROM [{0}]", "Deductee Details$"), excel_con))
                            {
                            ...
                            {
                                s = dtExcelData.Rows[1][0].ToString();
                                if (s == "(Mandatory)")
                                {
                            ...
                            SqlHelper.ExecuteNonQuery(_cnnString2, CommandType.StoredProcedure, "Usp_27Q_ExcelValidation", param);
                            ...
                            using (SqlConnection con = new SqlConnection(_cnnString2))
                            {
                                ...
                                using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                                {
                                    ...
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[5].ToString(), "Amount_Paid_Credited");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[6].ToString(), "TDS");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[7].ToString(), "Surcharge");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[8].ToString(), "Education_Cess");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[9].ToString(), "Total_Tax_Deducted");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[10].ToString(), "Rate_at_which_deducted");
                                    ...
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[13].ToString(), "TDS_Rate_Code");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[14].ToString(), "Remittance");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[15].ToString(), "Country");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[16].ToString(), "Contact_no");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[17].ToString(), "Email");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[18].ToString(), "Tax_Identification");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[19].ToString(), "Nri_Address");
