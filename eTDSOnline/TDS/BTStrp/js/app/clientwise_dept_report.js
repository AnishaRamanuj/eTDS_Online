...
$(function () {
    ...
    $("[id*=btnReport]").click(function () {
        //GetAllSelected();
        if ($("[id*=hdnSelectedcltCode]") == '') {
            showWarningAlert('Kindly Select atleast one Client !!!');
            return;
        } else {
            $("[id*=dvInvoice]").hide();
            $("[id*=dvReport]").show();
    ...
    $("#tblClient").on("click", "input[name=chkSelectPrimary]", function () {
        LoadDepartments();
    });
...
function BindPageLoadStaff() {
    ...
    $.ajax({
        type: "POST",
        url: "../Services/Clientwise_Dept_Report.asmx/Bind_DrpClient",
        data: '{compid:' + Compid + '}',
        dataType: 'json',
        ...
        success: function (msg) {
            ...
            if ($.isEmptyObject(checkboxListItems)) {
                ...
                tbl = tbl + "</tr>";
                $("[id*=tblClient]").append(tbl);
            }
            else {
                tbl = tbl + "<tr><td colspan='2'><input type='search' id='txtFilter' class='form-control' placeholder='Type to filter...' onkeyup='FilterList(checkboxListItems, this)' /></td></tr>";
                $("[id*=tblClient]").append(tbl);
...
function pageFiltersReset() {
    ...
    $("[id*=tblClient] tbody").empty();
    $("[id*=tblClient] thead").empty();
...
function LoadDepartments() {
    ...
    var req = {
        ...
        fromDate: frtime,
        toDate: totime,
        cltids: selectedIds.join(",")
    };
    Blockloadershow();
    $.ajax({
        type: "POST",
        url: "../Services/Clientwise_Dept_Report.asmx/Bind_DrpDept",
        ...
        success: function (msg) {
            ...
            if ($.isEmptyObject(checkboxListDepts)) {
                ...
                tbl = tbl + "<tr><td colspan='2'><input type='search' id='txtFilterDept' class='form-control' placeholder='Type to filter...' onkeyup='FilterDeptList(checkboxListDepts, this)' /></td></tr>";
                $("#tblDept").append(tbl);
...
function GetGrid() {
    ...
    $("[id*=hdnSelectedcltCode]").val(hdnSltEmp);
    $("[id*=hdnSelectedDeptCode]").val(selectedDeptsStr);
    $.ajax({
        type: "POST",
        url: "../Services/Clientwise_Dept_Report.asmx/GetDepartemtReport",
        data: JSON.stringify({ compid: compid, frtime: frtime, totime: totime, cltidids: hdnSltEmp, deptIds: selectedDeptsStr }),
        contentType: "application/json; charset=utf-8",
        success: function (msg) {
            ...
            tbl = tbl + "<thead><tr>";
            // tbl = tbl + "<th class='labelChange' style='font-weight: bold;'>Sr.No</th>";
            ...
            if (myList.length > 1) {
                ...
                for (var i = 0; i < myList.length; i++) {
                    tbl = tbl + "<tr>";
                    var isTotalRow = (myList[i].TaskDescription == 'Total' || myList[i].TaskDescription == 'Grand Total') &&
                        myList[i].DeptName == '' && myList[i].Date == '' && myList[i].DayName == '';
                    if (!isTotalRow) {
                        // tbl = tbl + "<td style='text-align: Center;'>" + myList[i].sino + "</td>";
                        ...
                        tbl += "<td style='text-align: right;font-weight: bold;' colspan='9'>" + myList[i].TaskDescription + "</td>";
