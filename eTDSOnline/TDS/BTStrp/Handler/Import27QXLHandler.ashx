<%@ WebHandler Language="C#" Class="Import27QXLHandler" %>
...
public class Import27QXLHandler : DataLayer.DALCommon, IHttpHandler, IRequiresSessionState
{
            ...
            if (context.Request.Files.Count > 0)
            {
                ...
                using (OleDbConnection excel_con = new OleDbConnection(conString))
                {
                    ...
                    for (int i = 0; i < dtSchema.Rows.Count; i++)
                    {
                        ...
                        if (Sheet1 == "'Deductee Details$'")
                        {
                            ...
                            SqlHelper.ExecuteNonQuery(_cnnString2, CommandType.StoredProcedure, "Usp_BTS_27Q_ExcelValidation", param);
                            if (dtExcelData.Rows.Count > 0)
                            {
                                isEmptyExcel = false;
                            }
                            using (SqlConnection con = new SqlConnection(_cnnString2))
                            {
                                using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                                {
                                    ...
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[14].ToString(), "TDS_Rate_Code");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[15].ToString(), "Remittance");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[16].ToString(), "Country");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[17].ToString(), "Contact_no");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[18].ToString(), "Email");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[19].ToString(), "Tax_Identification");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[20].ToString(), "Nri_Address");
                                    sqlBulkCopy.ColumnMappings.Add(dtExcelData.Columns[21].ToString(), "Opt115BAC");
                            ...
                            param[1] = new SqlParameter("@indication", 1);
                            param[2] = new SqlParameter("@formType", HttpContext.Current.Session["form"]);
                            ds = SqlHelper.ExecuteDataset(_cnnString2, CommandType.StoredProcedure, "Usp_BTS_27Q_ExcelValidation", param);
                            if (ds.Tables.Count > 2)
                            {
                                dsExport.Tables.Add(ds.Tables[0].Copy());
                ...
                else if (dsExport.Tables.Count > 4)
                {
                   ...
                   //     dAL.CreateLog("File uploaded For " + HttpContext.Current.Session["form"].ToString(), message, HttpContext.Current.Session["companyid"].ToString());
                        ...
                        SqlHelper.ExecuteNonQuery(_cnnString2, CommandType.StoredProcedure, "BTS_SyncChallan27QVoucherExcelInfo", param);
    ...
    private string DumpChallanVoucherExcel(DataSet ds, HttpContext context, bool isBackup)
    {
        ...
        templatefile =  HttpContext.Current.Server.MapPath("~/BTStrp/Templates/")+Convert.ToString(context.Session["form"])+ "_Error_XLSheet.xlsx";
