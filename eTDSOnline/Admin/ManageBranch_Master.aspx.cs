...
public partial class Admin_ManageBranch_Master : System.Web.UI.Page
{
    CommonLibrary.CommonFunctions ErrorException = new CommonLibrary.CommonFunctions();
    BAL_Branch_Master objBAL_Branch_Master = new BAL_Branch_Master ();
        ...
        private void BindGrid()
        {
            try
            {
                ...
                objBAL_Branch_Master._Company_ID = int.Parse(Session["companyid"].ToString());
                DataSet ds = objBAL_Branch_Master.Get_Branch_Master_List();
        ...
        private void FullFillValue(int Branch_ID)
        {
            try
            {
                objBAL_Branch_Master._Branch_ID = Branch_ID;
                objBAL_Branch_Master._Company_ID = int.Parse(Session["companyid"].ToString());
                DataSet ds = objBAL_Branch_Master.Get_Branch_Master_Details();
        ...
        private void DeleteBranch(int Branch_ID)
        {
            try
            {
                objBAL_Branch_Master._Branch_ID = Branch_ID;
                objBAL_Branch_Master._Company_ID = int.Parse(Session["companyid"].ToString());
                DataSet ds1 = objBAL_Branch_Master.Check_with_Deductee_Master();
                if (ds1.Tables[0].Rows.Count > 0)
                {
                    ucMessageControl.SetMessage("Delete Deductee First, Data Exist", MessageDisplay.DisplayStyles.Error);
                }
                else
                {

                    DataSet ds = objBAL_Branch_Master.Delete_Branch_Master();
        ...
        protected void btnSave_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    objBAL_Branch_Master._Branch_Name = txtBranch_Name.Text;
                   // objBAL_Branch_Salary_Master._State_ID = Convert.ToInt32(ddlState.SelectedValue);
                    objBAL_Branch_Master._Company_ID = int.Parse(Session["companyid"].ToString()); // Manual Company
                    //objBAL_Branch_Master._Created_by = 111; // Manual Company

                    ds = objBAL_Branch_Master.Insert_Branch_Master();
                }
                else
                {
                    objBAL_Branch_Master._Branch_ID = Convert.ToInt32(hdnBranch_ID.Value);
                    objBAL_Branch_Master._Branch_Name = txtBranch_Name.Text;
                    //objBAL_Branch_Master._State_ID = Convert.ToInt32(ddlState.SelectedValue);
                    objBAL_Branch_Master._Company_ID = int.Parse(Session["companyid"].ToString()); // Manual Company
                   // objBAL_Branch_Master._Created_by = 111; // Manual Company

                    ds = objBAL_Branch_Master.Update_Branch_Master();
                }
