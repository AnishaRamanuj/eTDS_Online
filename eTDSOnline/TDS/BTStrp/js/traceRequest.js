...
RequestTrace = function () {
    ...
    if (ddlQuarter == "" || ddlQuarter == null || ddlQuarter == undefined) {
        showDangerAlert('Select Quarter');
        return false;
    }
    if (ddlForm == "" || ddlForm == null || ddlForm == undefined) {
        showDangerAlert('Select Form');
        return false;
    }

    if (PRNNo == null || PRNNo == undefined || PRNNo == "") {
        showDangerAlert('Enter Token / PRN Number');
        return false;

    } else {
        if (isNaN(PRNNo)) {
            showDangerAlert('Token Number / PRN is a numerical field. Please ensure you enter only numerals');
            return false;
        }
    }
    //----------------------------------------------  
    ...
    if (DateOfDeposit != "") {
        ...
        if (!dtRegex.test(DateOfDeposit)) {
            showDangerAlert("Please enter dd-mmm-yyyy");
            return false;
        }
    }
    ...
    if ((PAN1 == null || PAN1 == undefined || PAN1 == "") && ((PAN2 != null && PAN2 != undefined && PAN2 != "") || (PAN3 != null && PAN3 != undefined && PAN3 != ""))) {
        showDangerAlert("Please selct PAN in first row before second or third row.");
        return false;
    }

    if ((PAN2 == null || PAN2 == undefined || PAN2 == "") && (PAN3 != null && PAN3 != undefined && PAN3 != "")) {
        showDangerAlert("Please selct PAN in second row before third row.");
        return false;
    }
    ...
    if (ChallanNo == null || ChallanNo == undefined || ChallanNo == "") {
        showDangerAlert('Enter Challan Serial No./ DDO');
        return false;
    }

    if (BSRCode == null || BSRCode == undefined || BSRCode == "") {
        showDangerAlert('Enter BSR Code / Receipt No.');
        return false;
    }

    if (TaxDeposited == null || TaxDeposited == undefined || TaxDeposited == "") {
        showDangerAlert('Enter Challan Amount / Transfer Voucher');
        return false;
    }
