...
public class Txt : DataLayer.DALCommon, IHttpHandler, IRequiresSessionState {
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            if (context.Request.Files.Count > 0)
            {
                ...
                foreach (DataRow row in dTtxt.AsEnumerable())
                {
                    ...
                    else if (rec == "DD")
                    {
                        ...
                        else
                        {
                            ...
                            ddr[25] = splitData[30].ToString(); // dbo.SplitIndex('^', @Sq, 31)
                            ddr[26] = splitData[31].ToString(); // dbo.SplitIndex('^', @Sq, 32)
                            ddr[27] = splitData[32].ToString();//splitData[33].ToString(); // dbo.SplitIndex('^', @Sq, 34)  TDS Section
                ...
                param[5] = new SqlParameter("@QHash", "");
